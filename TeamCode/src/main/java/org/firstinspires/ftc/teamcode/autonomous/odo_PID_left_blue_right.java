...
@Autonomous(name = "PID_LBR")
public class odo_PID_left_blue_right extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Pose2d targetPose = auto_constants.BLUE_LEFT_RIGHT_SPIKE;
        ...
        while (opModeIsActive()) {
            ...
            while (!isParked && opModeIsActive()){
                ...
                telemetry.addData("IsParked:", isParked);
                telemetry.update();
                if (Math.abs(calculator.errorX) <= 1 && Math.abs(calculator.errorY) <= 1 && Math.abs(calculator.errorHeading) <= 0.2)
                    isParked = true;
                Robot.DD.applySpeedFieldCentric(new vec2(relocation.getX(), relocation.getY()), relocation.getHeading(), myPose.getHeading());
            }
            Robot.DD.applySpeed(new vec2(0), 0, telemetry);
